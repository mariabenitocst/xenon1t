...
df_main = pd.DataFrame({'energy':a_mc_energy, 's1':a_s1, 's2':a_s2, 'cs1':a_cs1, 'cs2':a_cs2, 'pax_acceptances':a_pax_acceptances, 's1_cut_acceptances':a_s1_cut_acceptances, 's2_cut_acceptances':a_s2_cut_acceptances, 'threshold_acceptances':a_threshold_acceptances})
...
#df_main['threshold_acceptances'] *= df_main['cs1'].apply(s1_threshold)

d_main = df_main.to_dict()

bin_edges_energy = np.logspace(np.log10(l_energy_settings[1]), np.log10(l_energy_settings[2]), l_energy_settings[0]+1)
...
d_arrays['threshold_only']['l_bins_to_pop'] = []
...
d_arrays['total_minus_threshold']['a_num'] = np.zeros(l_energy_settings[0])
...
for i in tqdm.tqdm(xrange(l_energy_settings[0])):
    ...
    current_df = current_df[~((current_df['s1_cut_acceptances'] < 0) | (current_df['pax_acceptances'] < 0) | (current_df['s2_cut_acceptances'] < 0))]
    ...
    d_arrays['s1_cut_only']['a_num'][i] = num_events_s1_cuts
    ...
    d_arrays['s2_cut_only']['a_num'][i] = num_events_s2_cuts
    ...
    d_arrays['pax_only']['a_num'][i] = num_events_pax
    ...
    d_arrays['threshold_only']['a_num'][i] = num_events_threshold
    ...
    d_arrays['total_minus_threshold']['a_num'][i] = num_events_total_minus_threshold
    ...
    d_arrays['total']['a_num'][i] = num_events_total
...
fig_acceptances, (ax_acceptances, ax_num_events) = plt.subplots(2)
max_val = -1

for label in l_keys:
    spline_y = spline(d_arrays[label]['bin_centers'], d_arrays[label]['a_prob'], d_arrays[label]['bin_centers'])
    ax_acceptances.plot(d_arrays[label]['bin_centers'], d_arrays[label]['a_prob'], color=d_arrays[label]['color'], marker='o', linestyle='', label=d_arrays[label]['label'])
    ...
    ax_num_events.plot(d_arrays[label]['bin_centers'], d_arrays[label]['a_num'], color=d_arrays[label]['color'], marker='o', linestyle='', label=d_arrays[label]['label'])

    test_max_val = max(d_arrays[label]['a_num'])
    if test_max_val > max_val:
...
ax_num_events.set_title(r'SR0 Signal Event Count - %.1e Events' % (num_mc_events))
ax_num_events.set_xlabel(r'Energy [keV]')
ax_num_events.set_ylabel(r'Number of Events')

ax_acceptances.set_xlim(bin_edges_energy[0], bin_edges_energy[-1])
ax_acceptances.set_ylim(1./num_mc_events/2, 1.05)

ax_num_events.set_xlim(bin_edges_energy[0], bin_edges_energy[-1])
ax_num_events.set_ylim(1, max_val*1.1)

fig_acceptances.savefig('%sambe_efficiencies_%.3f_kV_linear.png' % (s_path_for_save, cathode_setting))

ax_acceptances.legend(loc='best', fontsize=6)
ax_acceptances.set_xscale('log')
ax_acceptances.set_yscale('log')

ax_num_events.set_xscale('log')
ax_num_events.set_yscale('log')

fig_acceptances.savefig('%sambe_efficiencies_%.3f_kV_log.png' % (s_path_for_save, cathode_setting))

fig_acceptances.tight_layout()

pickle.dump(df_main, open('%sdf_ambe_efficiencies_%.3f_kV_pickle.p' % (s_path_for_save, cathode_setting), 'w'))
