...
d_arrays = pickle.load(open('./mc_output/cnns_lax_0.11.1.p', 'r'))


fig_cnns, ax_cnns = plt.subplots(1)
...
cax_cnns = ax_cnns.pcolor(s1_edges, log_edges, a_hist_2d.T, cmap='Blues')
ax_cnns.axvline(x=roi_lb_cut_s1, color='r', linestyle='--')
fig_cnns.colorbar(cax_cnns, label=r'$\frac{Events}{Year}$', format='%.2e')


ax_cnns.set_title(r'CNNS Bkg')
ax_cnns.set_xlabel(r'S1 [PE]')
ax_cnns.set_ylabel(r'$Log_{10}(\frac{S2}{S1})$')

ax_cnns.text(0.45, 0.85, r'$\mathrm{Integrated\ Rate} = %.2e \ \frac{\mathrm{Events}}{\mathrm{Year}}$' % (np.sum(a_roi_hist2d)), transform=ax_cnns.transAxes, fontsize=10)

#ax_cnns.set_xlim(d_arrays[label]['bin_centers'][0], d_arrays[label]['bin_centers'][-1])
#ax_cnns.set_ylim(0, 1.05)

ax_cnns.legend(loc='best', fontsize=10)

fig_cnns.savefig('%scnns_%s.png' % (s_path_for_save, lax_version))
