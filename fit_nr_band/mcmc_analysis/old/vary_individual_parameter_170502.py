...
"""
num_pts_kde = int(5e3)
a_partial_sampler = np.zeros((num_pts_kde, num_dim))
for i in tqdm.tqdm(xrange(num_pts_kde)):
...
for i, par_name in enumerate(l_par_names):
    ...
    grid = grid_search.GridSearchCV(neighbors.KernelDensity(), {'bandwidth':np.linspace(0.01, 2., 20)}, cv=3, verbose=1, n_jobs=4)
    ...
    d_parameter_info[par_name]['best_fit'] = result.x[0]


print d_parameter_info
"""
...
gpu_observables_func = SourceModule(cuda_full_observables_production.cuda_full_observables_production_code, no_extern_c=True).get_function('gpu_full_observables_production_with_log_hist')
...
for sigma_level in l_sigma_levels:
    ...
    for i, par_name in enumerate(l_par_names):
        if not (par_name == par_to_examine):
            d_parameters_for_gpu[par_name] = np.asarray(d_all_parameter_info[par_name]['best_fit'], dtype=np.float32)
        else:
            d_parameters_for_gpu[par_name] = np.asarray(d_grid_parameter_percentiles[par_name][sigma_level], dtype=np.float32)
    ...
    d_grid_parameter_histograms[par_name][sigma_level]['mc_hist'] /= sum_mc
