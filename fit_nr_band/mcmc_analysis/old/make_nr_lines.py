...
fig_s1_log, (ax_s1_log_data, ax_s1_log_mc) = plt.subplots(2, sharex=False, sharey=False, figsize=figure_sizes)
...
l_lines_mc = list(np.arange(95, 4, -5))#[50, 10, 1] # quantiles for lines
l_lines_mc += [1]
print l_lines_mc
d_lines_mc = {}
for quantile in l_lines_mc:
    d_lines_mc[quantile] = [0 for i in xrange(l_s1_settings[0])]
...
for i in xrange(l_s1_settings[0]):
    ...
    for quantile in d_lines_mc:
        if len(current_df_mc['log']) == 0:
            d_lines_mc[quantile][i] = d_lines_mc[quantile][i-1]
            #print 'Need to simulate more points!'
            #ctx.pop()
            #sys.exit()
        else:
            d_lines_mc[quantile][i] = np.percentile(current_df_mc['log'], quantile)
...
for pt in l_missed_pts:
    ...
    for quantile in d_lines_mc:
        d_lines_mc[quantile].pop(pt)


# draw on data hist and get handles
line_handle_data, = ax_s1_log_data.plot(a_median_line_s1, a_median_line_data_log, color='r', label='Data $%d^{th}$ Percentile' % (50))
ax_s1_log_mc.plot(a_median_line_s1, a_median_line_data_log, color='r', label='Data $%d^{th}$ Percentile' % (50))

d_line_handles = {}
l_line_handles = []
for i, quantile in enumerate(l_lines_mc):
    if i % 3 == 0:
        line_style='-'
    elif i % 3 == 1:
        line_style='--'
    ...
    d_line_handles[quantile], = ax_s1_log_data.plot(a_median_line_s1, d_lines_mc[quantile], color='magenta', label=r'MC $%d^{%s}$ Percentile' % (quantile, s_raised), linestyle=line_style)
    ax_s1_log_mc.plot(a_median_line_s1, d_lines_mc[quantile], color='magenta', label=r'MC $%d^{%s}$ Percentile' % (quantile, s_raised), linestyle=line_style)

    l_line_handles.append(d_line_handles[quantile])



# add labels to plots
ax_s1_log_data.set_title(r'NR Band Data - $V_c$ = %.3f kV' % (cathode_setting), fontsize=12)
ax_s1_log_data.set_xlim(s1_edges[0], s1_edges[-1])
...
ax_s1_log_data.legend(handles=[line_handle_data] + l_line_handles, loc='best', fontsize=4)
...
d_median_line['mc_lines'] = {}
for quantile in d_lines_mc:
    d_median_line['mc_lines'][quantile] = d_lines_mc[quantile]

pickle.dump(d_median_line, open('%snr_band_lines.p' % (s_path_for_save), 'w'))


fig_s1_log.savefig('%snr_bands_%s_%.3f_kV.png' % (s_path_for_save, dir_specifier_name, cathode_setting))
