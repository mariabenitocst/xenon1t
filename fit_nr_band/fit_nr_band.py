...
class fit_nr(object):
    def __init__(self, d_fit_data, fit_type, num_mc_events, name_notes=None, l_gpus=[0], num_loops=1):
        ...
        self.d_acceptances = pickle.load(open('%sacceptances.p' % (config_xe1t.path_to_fit_inputs), 'r'))
        # PF acceptance
    ...
    def ln_likelihood_full_matching_band(self, w_value, alpha, zeta, beta, gamma, delta, kappa, eta, lamb, g1_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, dpe_prob, s1_bias_par, s1_smearing_par, s2_bias_par, s2_smearing_par, acceptance_par, scale_par, d_gpu_local_info, draw_fit=False):
        ...
        logLikelihoodMatching = c_log_likelihood(flat_s1_log_data, flat_s1_log_mc, len(flat_s1_log_data), int(self.d_coincidence_data_information['num_data_pts']), 0.95)
    ...
    def initial_positions_for_ensemble(self, a_free_parameters, num_walkers):
        ...
        for par_name in l_par_names:
            ...
            elif (par_name[-8:] == 'bias_par') or (par_name[-8:] == 'ring_par') or (par_name[:6] == 'accept'):
                d_variable_arrays[par_name] = np.random.normal(a_free_parameters[count_free_pars], a_free_parameters[count_free_pars]*0.3, size=num_walkers)
                count_free_pars += 1
