...
class fit_nr(object):
    ...
    def fill_energy_array(self, view_energy_spectrum=False):
        ...
        cdf = np.cumsum(self.d_mc_positions['xy_map'].T.ravel())
        cdf = cdf / cdf[-1]
        values = np.random.rand(self.num_mc_events)
        value_bins = np.searchsorted(cdf, values)
    ...
    def ln_likelihood_full_matching_band(self, w_value, alpha, zeta, beta, gamma, delta, kappa, eta, lamb, g1_value, extraction_efficiency_value, gas_gain_mean_value, gas_gain_width_value, dpe_prob, s1_bias_par, s1_smearing_par, s2_bias_par, s2_smearing_par, acceptance_par, cut_acceptance_par, prob_bkg, scale_par, d_gpu_local_info, draw_fit=False):
        ...
        if not draw_fit:
            ...
            root_numpy.array2hist(self.d_coincidence_data_information['a_log_s2_s1'].sum(axis=1), h_s1_data)
            ...
            root_numpy.array2hist(self.d_coincidence_data_information['a_log_s2_s1'].sum(axis=0), h_s2_data)
            ...
            neriX_analysis.save_plot(['temp_results'], c1, '1d_hists_%.3f_kV' % self.cathode_setting, batch_mode=True)
            f.savefig('./temp_results/2d_hist_%.3f_kV.png' % self.cathode_setting)
        ...
        logLikelihoodMatching = c_log_likelihood(flat_s1_log_data, flat_s1_log_mc, len(flat_s1_log_data), 1e-3)
    ...
    def run_mcmc(self, num_steps, num_walkers, num_threads=1, thin=1):
        ...
        if not loaded_prev_sampler:
            ...
            if self.fit_type == 'sb':
                # 292.2
                # free
                a_free_parameter_guesses = [1.21569448e-02, 1.33121774e-01, 4.36904667e-01, 3.96000240e-04, 8.49045992e-01, 8.68989011e-01, -1.89364189e+00, -1.97280558e+00, 2.34994120e-02, 9.29556297e-01]
                # 282.8
    ...
    def suppress_likelihood(self, iterations=200, a_free_par_guesses=None):
        ...
        if self.fit_type == 'sb' and a_free_par_guesses == None:
            ...
            # -868 (3.0)
            # prior on gamma and kappa
            a_guesses = [1.21569448e-02, 1.33121774e-01, 4.36904667e-01, 3.96000240e-04, 8.49045992e-01, 8.68989011e-01, -1.89364189e+00, -1.97280558e+00, 2.34994120e-02, 9.29556297e-01] # from diff ev
...
if __name__ == '__main__':
    ...
    l_parameters = []
    # [1.21569448e-02, 1.33121774e-01, 4.36904667e-01, 3.96000240e-04, 8.49045992e-01, 8.68989011e-01, -1.89364189e+00, -1.97280558e+00, 2.34994120e-02, 9.29556297e-01]
    ...
    l_parameters += [test.g1_value, test.extraction_efficiency_value, test.gas_gain_value, test.gas_gain_width, 0.2, 4.36904667e-01, 3.96000240e-04, 8.49045992e-01, 8.68989011e-01, -1.89364189e+00, -1.97280558e+00, 2.34994120e-02, 9.29556297e-01]
    test.gpu_pool.map(test.wrapper_ln_likelihood_full_matching_band, [l_parameters])
    
    
    #a_free_par_bounds = [(0.001, 0.04), (0.1, 0.2), (0, 1.), (0, 1.), (0, 1.), (0, 1.), (-2, 2), (-3, 3), (0., 0.4), (0.8, 1.)]
    ...
    #test.run_mcmc(num_steps=60, num_walkers=256)
