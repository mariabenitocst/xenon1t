	...
	if (iteration < *num_trials)
	{
        ...
        for (repetition_number=0; repetition_number < *num_loops; repetition_number++)
        {
            ...
            if (curand_uniform(&s) > *prob_bkg)
            {
                ...
                //printf("acc %f %f\\n", mcS2, s2_cut_acceptance);
                
                
                // ------------------------------------------------
                //  Apply S2 threshold cut
                // ------------------------------------------------
                
                //printf("s2 cut removed\\n");
                if (mcS2 < 76.)
                {
                    state[iteration] = s;
                    continue;
	...
	if (iteration < *num_trials)
	{
        ...
        for (repetition_number=0; repetition_number < 1; repetition_number++)
        {
            ...
            if (curand_uniform(&s) > *prob_bkg)
            {
                ...
                acceptance_probability *= s1_cut_acceptance*s2_cut_acceptance;
...
__global__ void gpu_full_observables_production_with_arrays_jelle(curandState *state, int *num_trials, float *meanField, float *aEnergy, float *a_x, float *a_y, float *a_z, float *a_e_survival_prob, float *prob_bkg, float *a_er_band_s1, float *a_er_band_log, int *num_spline_pts_yields, float *a_spline_energies, float *a_py, float *a_qy, float *g1Value, float *extractionEfficiency, float *gasGainValue, float *gasGainWidth, float *dpe_prob, float *s1_bias_par, float *s1_smearing_par, float *s2_bias_par, float *s2_smearing_par, float *acceptance_parameter, int *num_pts_s1bs, float *a_s1bs_s1s, float *a_s1bs_lb_bias, float *a_s1bs_ub_bias, float *a_s1bs_lb_smearing, float *a_s1bs_ub_smearing, int *num_pts_s2bs, float *a_s2bs_s2s, float *a_s2bs_lb_bias, float *a_s2bs_ub_bias, float *a_s2bs_lb_smearing, float *a_s2bs_ub_smearing, int *num_pts_s1pf, float *a_s1pf_s1s, float *a_s1pf_lb_acc, float *a_s1pf_mean_acc, float *a_s1pf_ub_acc, int *num_bins_r2, float *bin_edges_r2, int *num_bins_z, float *bin_edges_z, float *s1_correction_map, int *num_bins_x, float *bin_edges_x, int *num_bins_y, float *bin_edges_y, float *s2_correction_map, float *a_s1, float *a_s2)
{
        ...
        for (repetition_number=0; repetition_number < 1; repetition_number++)
        {
            ...
            if (curand_uniform(&s) > *prob_bkg)
            {
                ...
                */
                ...
                // ------------------------------------------------
                //  Find number of photons and electrons
                // ------------------------------------------------
                
                
                
                mcPhotons = mcEnergy*find_value_from_spline(mcEnergy, a_spline_energies, a_py, num_spline_pts_yields);
                //printf("py %f\\n", mcPhotons);
                mcElectrons = mcEnergy*find_value_from_spline(mcEnergy, a_spline_energies, a_qy, num_spline_pts_yields);
...
__global__ void gpu_full_observables_production_with_log_hist_no_fv(curandState *state, int *num_trials, float *meanField, float *aEnergy, float *a_x, float *a_y, float *a_z, float *a_e_survival_prob, float *prob_bkg, float *a_er_band_s1, float *a_er_band_log, float *w_value, float *alpha, float *zeta, float *beta, float *gamma, float *delta, float *kappa, float *eta, float *lambda, float *g1Value, float *extractionEfficiency, float *gasGainValue, float *gasGainWidth, float *dpe_prob, float *s1_bias_par, float *s1_smearing_par, float *s2_bias_par, float *s2_smearing_par, float *acceptance_parameter, int *num_pts_s1bs, float *a_s1bs_s1s, float *a_s1bs_lb_bias, float *a_s1bs_ub_bias, float *a_s1bs_lb_smearing, float *a_s1bs_ub_smearing, int *num_pts_s2bs, float *a_s2bs_s2s, float *a_s2bs_lb_bias, float *a_s2bs_ub_bias, float *a_s2bs_lb_smearing, float *a_s2bs_ub_smearing, int *num_pts_s1pf, float *a_s1pf_s1s, float *a_s1pf_lb_acc, float *a_s1pf_mean_acc, float *a_s1pf_ub_acc, int *num_pts_s1cuts, float *a_s1cuts_s1s, float *a_s1cuts_mean_acc, int *num_pts_s2cuts, float *a_s2cuts_s2s, float *a_s2cuts_mean_acc, int *num_bins_r2, float *bin_edges_r2, int *num_bins_z, float *bin_edges_z, float *s1_correction_map, int *num_bins_x, float *bin_edges_x, int *num_bins_y, float *bin_edges_y, float *s2_correction_map, int *num_bins_s1, float *bin_edges_s1, int *num_bins_log_s2_s1, float *bin_edges_log_s2_s1, float *hist_2d_array, int *num_loops)
{
	...
	if (iteration < *num_trials)
	{
        ...
        for (repetition_number=0; repetition_number < *num_loops; repetition_number++)
        {
            ...
            if (curand_uniform(&s) > *prob_bkg)
            {
                ...
                if (*acceptance_parameter < 0)
                {
                    acceptance_probability_width = acceptance_prob_mean - find_value_from_spline(mcS1, a_s1pf_s1s, a_s1pf_lb_acc, num_pts_s1pf);
                    //printf("low acc width %f\\n", acceptance_probability_width);
                }
                else
                    acceptance_probability_width = find_value_from_spline(mcS1, a_s1pf_s1s, a_s1pf_ub_acc, num_pts_s1pf) - acceptance_prob_mean;
                ...
                s1_cut_acceptance = find_value_from_spline(mcS1, a_s1cuts_s1s, a_s1cuts_mean_acc, num_pts_s1cuts);
                s2_cut_acceptance = find_value_from_spline(mcS2, a_s2cuts_s2s, a_s2cuts_mean_acc, num_pts_s2cuts);
...
__global__ void gpu_full_observables_production_with_arrays_no_fv_corrected_and_uncorrected(curandState *state, int *num_trials, float *meanField, float *aEnergy, float *a_x, float *a_y, float *a_z, float *a_e_survival_prob, float *prob_bkg, float *a_er_band_s1, float *a_er_band_log, float *w_value, float *alpha, float *zeta, float *beta, float *gamma, float *delta, float *kappa, float *eta, float *lambda, float *g1Value, float *extractionEfficiency, float *gasGainValue, float *gasGainWidth, float *dpe_prob, float *s1_bias_par, float *s1_smearing_par, float *s2_bias_par, float *s2_smearing_par, float *acceptance_parameter, int *num_pts_s1bs, float *a_s1bs_s1s, float *a_s1bs_lb_bias, float *a_s1bs_ub_bias, float *a_s1bs_lb_smearing, float *a_s1bs_ub_smearing, int *num_pts_s2bs, float *a_s2bs_s2s, float *a_s2bs_lb_bias, float *a_s2bs_ub_bias, float *a_s2bs_lb_smearing, float *a_s2bs_ub_smearing, int *num_pts_s1pf, float *a_s1pf_s1s, float *a_s1pf_lb_acc, float *a_s1pf_mean_acc, float *a_s1pf_ub_acc, int *num_pts_s1cuts, float *a_s1cuts_s1s, float *a_s1cuts_mean_acc, int *num_pts_s2cuts, float *a_s2cuts_s2s, float *a_s2cuts_mean_acc, int *num_bins_r2, float *bin_edges_r2, int *num_bins_z, float *bin_edges_z, float *s1_correction_map, int *num_bins_x, float *bin_edges_x, int *num_bins_y, float *bin_edges_y, float *s2_correction_map, float *a_s1, float *a_s2, float *a_cs1, float *a_cs2, float *a_acceptances)
{
