...
s_path_to_input = './resources/Xe1T_FirstResults_AC_background.root'
#s_path_to_input = './resources/MCERBandForNRFitting_prel.root'
s_path_to_plots = './plots/supporting/ac_bkg/'
...
for i in tqdm.tqdm(xrange(num_random_events)):
    h_er.GetRandom2(x, y)
    a_s1[i], a_log[i] = float(x), float(np.log10(10.**y/x))


s1_bin_edges = np.linspace(config_xe1t.l_s1_settings[1], config_xe1t.l_s1_settings[2], config_xe1t.l_s1_settings[0]*2+1)
log_bin_edges = np.linspace(config_xe1t.l_log_settings[1], config_xe1t.l_log_settings[2], config_xe1t.l_log_settings[0]*2+1)
...
fig_er.savefig('%sac_bkg.png' % (s_path_to_plots))

d_er_band = {}
d_er_band['ac_bkg_hist'] = a_er_band_hist
d_er_band['ac_bkg_s1_edges'] = a_er_band_s1_edges
d_er_band['ac_bkg_log_edges'] = a_er_band_log_edges


pickle.dump(d_er_band, open('%sac_bkg.p' % (s_path_to_pickle_save), 'w'))
