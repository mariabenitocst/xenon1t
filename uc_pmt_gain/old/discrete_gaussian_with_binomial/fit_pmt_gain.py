...
class fit_pmt_gain(object):
    def __init__(self, filename, run=16, channel_number=17, num_mc_events=1e6, num_loops=1, b_making_comparison_plots=False):
        ...
        if self.file_identifier == '0062_0061':
            self.d_fit_files['settings'] = [250, -1e6, 2e7]
            self.a_free_par_guesses = [0.936, 5.01, 0.732, 4.37e4, 3.05e5, 1.17, 1.000]
            #self.a_free_par_guesses = [0.936, 5.01, 0.732, 4.37e4, 3.05e5, 1.17, 1.000]
        
        elif self.file_identifier == '0066_0065':
            self.d_fit_files['settings'] = [250, -1e6, 1.2e7]
            self.a_free_par_guesses = [9.37198905e-01, 4.76784135e+00, 5.39736775e-02, 7.33877167e-01, 1.35616433e+04, 2.76806428e+05, 1.14771975e+00, 1.00104460e+00]
            #self.a_free_par_guesses = [0.999, 5.00, 0.7029, 2.055e3, 2.68e5, 4.00e5, 0.140, 1.1027]
            
        elif self.file_identifier == '0067_0068':
            self.d_fit_files['settings'] = [250, -1e6, 7.5e6]
            self.a_free_par_guesses = [9.47052907e-01, 4.45060960e+00, 2.85023359e-01, 7.48623433e-01, -4.35274825e+03, 2.63771368e+05, 1.14453678e+00, 9.97325059e-01]
            #self.a_free_par_guesses = [0.9951, 4.082, 0.8275, 1.225e3, 2.64e5, 1.878e6, 0.187, 0.972]
            
        elif self.file_identifier == '0071_0072':
            self.d_fit_files['settings'] = [250, -1e6, 3.4e7]
            self.a_free_par_guesses = [0.92, 5.10, 0.750, 5.71e4, 3.22e5, 1.19, 0.992]
            #self.a_free_par_guesses = [0.92, 5.10, 0.750, 5.71e4, 3.22e5, 1.19, 0.992]
        
        elif self.file_identifier == '0073_0074':
            self.d_fit_files['settings'] = [250, -1e6, 4.2e7]
            self.a_free_par_guesses = [9.34339174e-01, 5.39380822e+00, 2.67071213e-01, 7.01781350e-01, 9.89362252e+04, 5.02272766e+05, 2.00556933e+00, 9.93418249e-01]
            #self.a_free_par_guesses = [0.982, 5.028, 0.76, 2.19e4, 2.84e5, 6.67e5, 0.193, 2.]

        elif self.file_identifier == 'nerix_160418_1523':
            self.d_fit_files['settings'] = [50, -5e5, 3.e6]
            self.a_free_par_guesses = [0.834, 14.25, 0.0157, 0.220, 5.55e4, 2.21e5, 1.33, 1.00]

        elif self.file_identifier == 'nerix_160418_1531':
            self.d_fit_files['settings'] = [50, -5e5, 4.e6]
    ...
    def cascade_model_ln_likelihood(self, a_parameters):
        ...
        l_args_gpu = [self.rng_states, drv.In(num_trials), drv.In(self.num_loops), drv.InOut(a_hist), drv.In(mean_num_pe), drv.In(prob_hit_first), drv.In(mean_e_from_dynode), drv.In(width_e_from_dynode), drv.In(probability_electron_ionized), drv.In(bkg_mean), drv.In(bkg_std), drv.In(num_bins), drv.In(bin_edges)]
    ...
    def run_cascade_model_mcmc(self, num_walkers=32, num_steps=2000, threads=1):
        ...
        l_std_guesses = 0.03*l_value_guesses #[0.05, 1.5, 0.1, 0.1, 1e4, 5e4, 0.2, 0.05]
        # self.a_free_par_guesses = [0.876, 9.44, 0.335, 9.25e4, 2.30e5, 2.354, 1.000]
        l_par_names = ['p_hit_first_dynode', 'electrons_per_dynode', 'p_e_freed', 'mean_num_pe']
    ...
    def draw_fit_with_peaks(self, num_walkers, num_steps_to_include):
        ...
        for i, num_pe in enumerate(l_num_pe):
            ...
            l_args_gpu = [self.rng_states, drv.In(num_trials), drv.In(self.num_loops), drv.InOut(current_hist), drv.In(num_pe), drv.In(prob_hit_first), drv.In(mean_e_from_dynode), drv.In(width_e_from_dynode), drv.In(probability_electron_ionized), drv.In(bkg_mean), drv.In(bkg_std), drv.In(num_bins), drv.In(bin_edges)]
    ...
    def suppress_likelihood(self, iterations=2000):
        ...
        var_ll = np.std(l_log_likelihoods, ddof=1)

        print 'Mean for %.3e MC iterations is %f' % (self.num_mc_events*self.num_loops, np.mean(l_log_likelihoods))
        print 'Standard deviation for %.3e MC iterations is %f' % (self.num_mc_events*self.num_loops, var_ll)
...
if __name__ == '__main__':
    ...
    #filename = 'darkbox_spectra_0066_0065' # DE-ln(L) = 540
    #filename = 'darkbox_spectra_0067_0068' # DE-ln(L) = 222.53
    filename = 'darkbox_spectra_0073_0074' # DE-ln(L)  = 1172
    
    
    [  9.16069615e-01,   4.66847537e+00,   7.26189473e-01,
         8.16076073e-01,   9.98560623e+04,   4.11941744e+05,
         2.07743623e+00,   9.91632240e-01]


    test = fit_pmt_gain(filename, num_mc_events=1e4, num_loops=10, b_making_comparison_plots=True)
    test.draw_mc_model_fit([9.16069615e-01, 4.66847537e+00, 7.26189473e-01, 8.16076073e-01, 9.98560623e+04, 4.11941744e+05, 2.07743623e+00, 9.91632240e-01])
    #print test.cascade_model_ln_likelihood(test.a_free_par_guesses)
    #test.draw_mc_model_fit(test.a_free_par_guesses)

    #test.draw_cascade_model_with_error_bands(num_walkers=64, num_steps_to_include=10)
    #test.draw_fit_with_peaks(num_walkers=64, num_steps_to_include=250)

    #a_bounds = [(0.75, 1), (1, 8), (0.01, 1.5), (0, 1.0), (-1e5, 1e5), (5    e4, 8e5), (0.6, 3.), (0.8, 1.2)]
    #test.differential_evolution_minimizer(a_bounds, maxiter=150, tol=0.05, popsize=20, polish=False)

    #test.suppress_likelihood()
    #test.run_cascade_model_mcmc(num_walkers=64, num_steps=50, threads=1)
